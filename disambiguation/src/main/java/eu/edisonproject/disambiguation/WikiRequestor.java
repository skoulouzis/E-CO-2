...
import static eu.edisonproject.disambiguation.Wikipedia.EXCLUDED_CAT;
...
public class WikiRequestor implements Callable {
  ...
  @Override
  public Map<CharSequence, List<CharSequence>> call() throws Exception {
//        long start = System.currentTimeMillis();
    Map<CharSequence, List<CharSequence>> map = null;
    switch (type) {
      case 0:
        map = getWikiCategory();
        break;
...
//        System.err.println(this.hashCode() + " Elapsed: " + (System.currentTimeMillis() - start));
  ...
  private Map<CharSequence, List<CharSequence>> getWikiCategory() throws IOException, ParseException {
    List<CharSequence> categoriesList = new ArrayList<>();
...
//                    System.err.println(cat.substring("Category:".length()).toLowerCase());
  ...
  private Map<CharSequence, List<CharSequence>> getWikidataNumProperty() throws IOException, ParseException {
    String jsonString = IOUtils.toString(url);
    Map<CharSequence, List<CharSequence>> map = new HashMap<>();
    JSONObject jsonObj = (JSONObject) JSONValue.parseWithException(jsonString);
...
//                System.err.println(id + " -> Q" + numericID);
  ...
  private Map<CharSequence, List<CharSequence>> getWikidataLables() throws IOException, ParseException {
    String jsonString = IOUtils.toString(url);
    Map<CharSequence, List<CharSequence>> map = new HashMap<>();
    JSONObject jsonObj = (JSONObject) JSONValue.parseWithException(jsonString);
