...
DICTIONARY_ALL=$HOME/workspace/E-CO-2/etc/allTerms.csv
...
do
  ...
  #do
        ...
        #echo res $res
        #echo len $resLen
        echo "screen -dmS $base prun -reserve ${res[$index]} -np 1 java -Dstop.words.file=$STOPWORDS -Ditemset.file=$DICTIONARY_ALL -Dmodel.path=$MODEL_PATH -jar $JAR_PATH -op x -i $i -o $i/terms.csv -p $PROPS_FILE"
