...
do
  ...
  do
        ...
        #echo res $res
        #echo len $resLen
        echo "screen -dmSL $base prun -reserve ${res[$index]} -Dstop.words.file=$STOPWORDS -Ditemset.file=$DICTIONARY_ALL -Dmodel.path=$MODEL_PATH -Dtagger.file=$TAGGER_FILE -jar $JAR_PATH -op x -i $f -o $i/terms.csv -p $PROPS_FILE"
  done
done
